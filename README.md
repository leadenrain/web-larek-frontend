...
- `constructor(baseUrl: string, options: RequestInit = {})` - конструктор, используя переданные аргументы, инициализирует свойства `baseUrl`(базовый адрес сервера) и `options`(глобальные настройки запросов) экземпляра класса.
Методы: 
