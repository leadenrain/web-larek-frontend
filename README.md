...
#### Page
Класс для управления элементами страницы.
- `_wrapper: HTMLDivElement` - обертка для контента страницы
- `_basketIcon: HTMLButtonElement` - иконка корзины
- `_counter: HTMLSpanElement` - счетчик на иконке корзины
- `_products: HTMLElement` - каталог товаров
- `set catalog: (products: HTMLElement[]) => void` - установка каталога на страницу
- `set counter: (value: number) => void` - установка значения счетчика
- `set locking(value: boolean) => void` - блокировка станицы                
...
- `_title: _title: HTMLHeadingElement` - название товара 
...
#### FormView
Класс для отображения форм заказа на странице. Так как в приложении форма заказа разбита на 2 части, конструктор принимает шаблонный элемент, чтобы избежать дублирования кода. Свойства класса:
